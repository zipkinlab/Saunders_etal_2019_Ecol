...
cat("
    ...
    harvest[1] ~ dnorm(161459,9.551e-10) # mean, 1/sd2 from 2004-2013 data, eastern
    harvest[2] ~ dnorm(436095,1.822e-10) # central
...
inits <- function(){list(pi=pi.inits, harvest=c(130000,370000))}   #harvest inits based on priors (means of 2004-2013)
...
# call jags
AMWO.harvest.wings.34cor.HPCC <- jagsUI(bugs.data, inits=inits, parameters, "AMWO.harvest.wings.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel=TRUE, store.data=TRUE)
load(file="AMWO_harvest_wings_deterministic_model_HPCC.Rda")
#this was run on the HPCC, close to convergence. Using these estimates for now, but will change when harvest model is revised by Todd.
...
cat("
    ...
    n[1,1,3,1] ~ dunif(1000000, 4000000)                  # initial size for female eastern (dnorm(600000,1E-10))
    n[1,1,3,2] ~ dunif(1000000, 4000000)                  # initial size for female central (dnorm(720000,1E-10))
    n[1,1,2,1] ~ dunif(1000000, 4000000)                  # initial size for male eastern (dnorm(530000,1E-10))
    n[1,1,2,2] ~ dunif(1000000, 4000000)                  # initial size for male central (dnorm(615000,1E-10))
    ...
    } #c                                              # note!! that reporting rate will change to vector of proportions of 1800 bands used each year
    } #t           
    } #p
    } # end bugs model
    ",fill = TRUE)
...
for(c in 1:3){
  for(p in 1:2){
    for(t in 1:51){
      pi[t,c,p] <- AMWO.harvest.wings.34cor.HPCC$mean$pi[t,c,p]
      H[t,c,p] <- round(AMWO.harvest.wings.34cor.HPCC$mean$H[t,c,p])
    }
  }
}
...
n.inits[1,1,3,2] <- 3500000
n.inits[1,1,2,1] <- 1200000
n.inits[1,1,2,2] <- 2200000
...
# Parameters monitored
parameters <- c("sa.x", "ss.x", "f.x", "sa.sd", "ss.sd", "f.sd", "sa", "f", "N")  # anything else to include? h?
...
print(AMWO.combo.jags, digits=3)
